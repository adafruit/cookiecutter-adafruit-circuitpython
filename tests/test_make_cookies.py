# SPDX-FileCopyrightText: 2024 Scott Shawcroft, written for Adafruit Industries
#
...
def is_pre_commit_clean(cookie_dir):
    ...
    subprocess.check_call(
        ["pre-commit", "run", "--all-files", "--show-diff-on-failure"], cwd=cookie_dir
    )
    return True
...
def compare_template_dirs(*, library_name="test", library_prefix=None):
    ...
    :param library_name: Name used for the template's `library_name` prompt.
    :param library_prefix: The value used for the template's `library_prefix` prompt.
    """
    ...
    for file in cookie_template_path.iterdir():
        if "{" in file.name:
            if not library_prefix:
                base_name = file.with_name(f"CircuitPython_{library_name}")
                template_files.add(base_name.with_suffix(file.suffix).name)
            else:
                base_name = file.with_name(
