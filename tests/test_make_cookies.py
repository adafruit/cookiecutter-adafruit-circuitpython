...
def overwrite_workaround():
    """ cookiecutter's `overwrite_if_exists=True` is currently broken.
        There is an accepted PR slated for the release 1.8.0, but until
...
def test_new_cookiecutter_only_required_entries(capsys):
    """ Basic test of running cookiecutter, supplying only the required
        info.
    """
...
def test_new_cookiecutter_all_entries(capsys):
    """ Basic test of running cookiecutter, supplying only the required
        info.
    """
